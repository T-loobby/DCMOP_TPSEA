function [return_pop,return_Fitness,allindex] = Auxiliray_task_EnvironmentalSelection2(Population,N,VAR)
...
elseif length(fPopulation) <= N
    ...
    % Sort the population
    [fFitness,rank1] = sort(fFitness);
    fPopulation = fPopulation(rank1);
    %
    if ~isempty(ifPopulation)
        ifFitness = CalFitness2(ifPopulation.objs,ifPopulation.cons); % ,
        Next2 = ifFitness < 1;
        ...
        allindex=[findex(rank1);ifindex(rank)];
    else
        allindex=findex(rank1);
        ifPopulation = [];
        ifFitness = [];
    end
